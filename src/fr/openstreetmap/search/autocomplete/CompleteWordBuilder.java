...
/**
 * Build the data for an optionally typo-tolerant but full-word searcher
 */
public class CompleteWordBuilder extends IndexBuilder {
	...
	public CompleteWordBuilder(File outputDir) throws IOException {
	...
	public void addEntry(List<ScoredToken> scoredTokens, byte[] data) throws IOException {
		...
		for (ScoredToken sc : scoredTokens) {
			temporaryFileWriter.write(sc.token + "\t" + offset + "\t" + sc.score + "\n");
		}
	}
